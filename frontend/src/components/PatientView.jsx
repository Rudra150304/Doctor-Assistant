...
function PatientView({ user, onSwitchRole }) {

  const SESSION_ID = `patient_${user?.email}`;

  const [messages, setMessages] = useState([
    { role: 'assistant', text: `Hello ${user?.name}! How can I help you today?` }
  ]);
        ...
        <div className="chat-header patient-header">
          <div className="header-left">
            ...
            <div>
              ...
              <span className="header-sub">{user?.name}</span>
            </div>
          </div>
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`msg-wrapper ${msg.role}`}>
              <div className="msg-bubble">
                {msg.role === 'assistant' && (
                  <span className="msg-icon patient-icon">🏥</span>
                )}
                <p>{msg.text}</p>
              </div>
            </div>
